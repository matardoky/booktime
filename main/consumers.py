...
class ChatNotifyConsumer(AsyncHttpConsumer):
    ...
    async def handle(self,body):
        ...
        if is_employee:
            logger.info(
                "Opening notify for user %s and params %s",
                self.scope.get("user"),
...
class ChatConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if user_type == ChatConsumer.EMPLOYEE:
            logger.info(
                "Opening chat stream for employee %s", self.scope["user"]
            )
            authorized = True
            ...
            logger.info(
                "Unauthorized connection from %s", self.scope["user"]
            )
            await self.close()
    ...
    async def disconnect(self, close_code):
        if not self.scope["user"].is_anonymous:
            ...
            logger.info(
                "Closing chat stream for user %s", self.scope["user"],
            )
...
class OrderTrackerConsumer(AsyncHttpConsumer):
    ...
    async def query_remote_server(self, order_id):
        async with aiohttp.ClientSession() as session:
            async with session.get("https://pastebin.com/raw/S8LYZ65S") as resp:
